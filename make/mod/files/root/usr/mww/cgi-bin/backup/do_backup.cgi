...
fname=$(echo ${CONFIG_PRODUKT_NAME}_${CONFIG_VERSION_MAJOR}.${CONFIG_VERSION}${CONFIG_LABOR_ID_NAME:+-$(/etc/version --project 2>/dev/null)}-$(cat /etc/.freetz-version)$(date '+_%Y-%m-%d_%H%M_settings.tgz') | tr ' !' '_.')
CR=$'\r'
