$(call PKG_INIT_BIN,$(if $(FREETZ_PACKAGE_SUBVERSION_VERSION_1_9),1.9.3,$(if $(FREETZ_PACKAGE_SUBVERSION_VERSION_1_8),1.8.15,1.7.22)))
$(PKG)_MAJOR_VERSION:=1
$(PKG)_SOURCE:=$(pkg)-$($(PKG)_VERSION).tar.bz2
$(PKG)_SOURCE_SHA1_1.7.22:=6b883a0e970262f6485fc20fef68a2afcffe0875
$(PKG)_SOURCE_SHA1_1.8.15:=680acf88f0db978fbbeac89ed63776d805b918ef
$(PKG)_SOURCE_SHA1_1.9.3 :=27e8df191c92095f48314a415194ec37c682cbcf
...
# Libraries
$(PKG)_LIBNAMES_SHORT := client delta diff fs fs_fs fs_util ra ra_local ra_svn repos subr wc
$(PKG)_LIBNAMES_SHORT += $(if $(FREETZ_PACKAGE_SUBVERSION_VERSION_1_7),ra_neon)
$(PKG)_LIBNAMES_SHORT += $(if $(or $(FREETZ_PACKAGE_SUBVERSION_VERSION_1_8),$(FREETZ_PACKAGE_SUBVERSION_VERSION_1_9)),ra_serf)
$(PKG)_LIBNAMES_SHORT += $(if $(FREETZ_PACKAGE_SUBVERSION_WITH_LIBDB),fs_base)
$(PKG)_LIBNAMES_SHORT += $(if $(FREETZ_PACKAGE_SUBVERSION_VERSION_1_9),fs_x)
...
$(PKG)_DEPENDS_ON += $(if $(FREETZ_PACKAGE_SUBVERSION_VERSION_1_7),neon)
$(PKG)_DEPENDS_ON += $(if $(or $(FREETZ_PACKAGE_SUBVERSION_VERSION_1_8),$(FREETZ_PACKAGE_SUBVERSION_VERSION_1_9)),serf)
...
$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_SUBVERSION_VERSION_1_7
$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_SUBVERSION_VERSION_1_8
$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_SUBVERSION_VERSION_1_9
...
$(PKG)_CONFIGURE_OPTIONS += $(if $(FREETZ_PACKAGE_SUBVERSION_VERSION_1_7),--with-neon="$(TARGET_TOOLCHAIN_STAGING_DIR)/usr" --disable-neon-version-check)
$(PKG)_CONFIGURE_OPTIONS += $(if $(or $(FREETZ_PACKAGE_SUBVERSION_VERSION_1_8),$(FREETZ_PACKAGE_SUBVERSION_VERSION_1_9)),--with-serf="$(TARGET_TOOLCHAIN_STAGING_DIR)/usr",--with-serf=no)
...
$($(PKG)_LIBS_STAGING_DIR) $($(PKG)_BINARIES_STAGING_DIR) $($(PKG)_MODULES_STAGING_DIR): $($(PKG)_DIR)/.compiled
	$(SUBMAKE1) -C $(SUBVERSION_DIR) \
		...
		install $(if $(or $(FREETZ_PACKAGE_SUBVERSION_VERSION_1_8),$(FREETZ_PACKAGE_SUBVERSION_VERSION_1_9)),install-tools install-mods-shared)
	$(PKG_FIX_LIBTOOL_LA) \
