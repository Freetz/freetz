$(call PKG_INIT_BIN,$(if $(FREETZ_PACKAGE_SUBVERSION_VERSION_1_8),1.8.14,1.7.22))
...
$(PKG)_LIBNAMES_SHORT += $(if $(FREETZ_PACKAGE_SUBVERSION_VERSION_1_8),ra_serf)
...
$(PKG)_DEPENDS_ON += $(if $(FREETZ_PACKAGE_SUBVERSION_VERSION_1_8),serf)
...
$(PKG)_CONFIGURE_OPTIONS += $(if $(FREETZ_PACKAGE_SUBVERSION_VERSION_1_8),--with-serf="$(TARGET_TOOLCHAIN_STAGING_DIR)/usr",--with-serf=no)
...
$($(PKG)_LIBS_STAGING_DIR) $($(PKG)_BINARIES_STAGING_DIR) $($(PKG)_MODULES_STAGING_DIR): $($(PKG)_DIR)/.compiled
	$(SUBMAKE1) -C $(SUBVERSION_DIR) \
		...
		install $(if $(FREETZ_PACKAGE_SUBVERSION_VERSION_1_8),install-tools install-mods-shared)
	$(PKG_FIX_LIBTOOL_LA) \
