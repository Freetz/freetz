...
if [ "${QUERY_STRING}" != "" ]; then
	...
	if [ "$DELETE" = "true" ]; then
		...
		if [ -f /var/dtmfbox/tmp/webinterface.pid ]; then
	...
	case "$START" in
		...
		daemon)
			# Daemon starten
			/mod/etc/rc.dtmfbox restart > /dev/null
			;;
		logged)
			# Daemon geloggt starten
			/mod/etc/rc.dtmfbox stop >/dev/null
			rm /var/dtmfbox/tmp/dtmfbox.log 2>/dev/null
			/mod/etc/rc.dtmfbox log >/dev/null
...
get_am_messages() {
  ...
  while [ $tmp_acc_no -ge 1 ]; do
	let msg_cnt=0;
	for file in $(find $DTMFBOX_PATH/record/${tmp_acc_no}/* 2>/dev/null); do
		if [ ! -d $file ]; then
...
####################################################################################
if [ "$PAGE" = "reset" ] || [ "$PAGE" = "reset_path_only" ]; then
...
if [ "$PAGE" = "am_messages" ]; then
	...
	if [ -f /var/dtmfbox/script/funcs.sh ]; then
