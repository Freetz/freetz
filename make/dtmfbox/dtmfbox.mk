...
$(PKG)_CFLAGS+=-DPJMEDIA_HAS_G711_CODEC=$(if $(DS_PACKAGE_$(PKG)_WITH_G711_CODEC),1,0)
$(PKG)_CFLAGS+=-DPJMEDIA_HAS_GSM_CODEC=$(if $(DS_PACKAGE_$(PKG)_WITH_GSM_CODEC),1,0)
#$(PKG)_CFLAGS+=-DPJMEDIA_HAS_ILBC_CODEC=$(if $(DS_PACKAGE_$(PKG)_WITH_ILBC_CODEC),1,0)
#$(PKG)_CFLAGS+=-DPJMEDIA_HAS_SPEEX_CODEC=$(if $(DS_PACKAGE_$(PKG)_WITH_SPEEX_CODEC),1,0)
...
$(PKG)_LDLIBS+=$(if $(DS_PACKAGE_$(PKG)_WITH_SPEEX_CODEC),-lspeex,)
$(PKG)_LDLIBS+=$(if $(DS_PACKAGE_$(PKG)_WITH_GSM_CODEC),-lgsmcodec,)
$(PKG)_LDLIBS+=$(if $(DS_PACKAGE_$(PKG)_WITH_ILBC_CODEC),-lilbccodec,)
...
$($(PKG)_BINARY): $($(PKG)_DIR)/.configured
	...
	$(if $(DS_PACKAGE_DTMFBOX_WITH_WEBPHONE),wget -O $(DTMFBOX_TARGET_DIR)/root/usr/mww/sWebPhone.jar $(DTMFBOX_WEBPHONE),)
	PATH="$(TARGET_PATH)" \
