/*
 * Freetz (http://freetz.org) info:
 *
 * This source file (code borrowed from Asterisk) was sent to me (kriegaex) by
 * Vladislav Grishenko via private e-mail as an answer to my inquiry on the
 * BusyBox mailing list, see
 * http://lists.busybox.net/pipermail/busybox/2011-December/077039.html
 *
 * It efficiently determines the external IP by querying a public STUN server.
 * Thus, this binary can optionally be used by Freetz's /usr/bin/get_ip as an
 * alternative to querying a site like http://checkip.dyndns.org via wget.
 *
 * STUN support code
 *
