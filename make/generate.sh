...
PKGS=$(
for dir in $(find "$MYPWD" -maxdepth 1 -mindepth 1 -type d); do
pkg=${dir##*/}
echo "$pkg" | grep -qE "^(busybox|libs|linux)$" && continue
cat="$(sed -n 's/^$(PKG)_CATEGORY *:= *//p' $dir/$pkg.mk)"
...
L="$(grep -P '^[ \t]*help[ \t]*$' -nm1 "$MYPWD/$pkg/Config.in" | sed 's/:.*//')"
C="$(wc -l "$MYPWD/$pkg/Config.in" | sed 's/ .*//')"
