...
$(PKG)_CONFIG_SUBOPTS += FREETZ_OPENSSH_STATIC
$(PKG)_CONFIG_SUBOPTS += FREETZ_OPENSSH_CLIENT
$(PKG)_CONFIG_SUBOPTS += FREETZ_OPENSSH_CLIENTUTILS
$(PKG)_CONFIG_SUBOPTS += FREETZ_OPENSSH_KEYUTILS
$(PKG)_CONFIG_SUBOPTS += FREETZ_OPENSSH_SFTP
$(PKG)_CONFIG_SUBOPTS += FREETZ_OPENSSH_SFTPCLIENT
$(PKG)_CONFIG_SUBOPTS += FREETZ_OPENSSH_SSHD



$(PKG)_CONFIGURE_OPTIONS += $(if $(FREETZ_OPENSSH_STATIC),--disable-shared,--enable-shared)
$(PKG)_CONFIGURE_OPTIONS += $(if $(FREETZ_OPENSSH_STATIC),--enable-static,--disable-static)
...
$(OPENSSH_CLIENT_SSH_BINARY) \
	...
	PATH="$(TARGET_PATH)" \
	$(MAKE) LDFLAGS="$(OPENSSH_LDFLAGS)" LIBS="$(OPENSSH_LIBS)" -C $(OPENSSH_DIR) \
		all

# Client binaries (ssh and scp)
$(OPENSSH_CLIENT_SSH_TARGET_BINARY): $(OPENSSH_CLIENT_SSH_BINARY)
ifeq ($(strip $(FREETZ_OPENSSH_CLIENT)),y)
...
ifeq ($(strip $(FREETZ_OPENSSH_CLIENTUTILS)),y)
	$(INSTALL_BINARY_STRIP)
else
...
ifeq ($(strip $(FREETZ_OPENSSH_KEYUTILS)),y)
	$(INSTALL_BINARY_STRIP)
else
...
ifeq ($(strip $(FREETZ_OPENSSH_SSHD)),y)
	$(INSTALL_BINARY_STRIP)
...
ifeq ($(strip $(FREETZ_OPENSSH_SFTP)),y)
	$(INSTALL_BINARY_STRIP)
else
