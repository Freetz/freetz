$(call PKG_INIT_BIN,$(if $(FREETZ_BUSYBOX__VERSION_V127),1.27.2,$(if $(FREETZ_BUSYBOX__VERSION_V131),1.31.1,1.32.0)))
$(PKG)_SOURCE:=$(pkg)-$($(PKG)_VERSION).tar.bz2
$(PKG)_SOURCE_MD5_1.27.2:=476186f4bab81781dab2369bfd42734e
$(PKG)_SOURCE_MD5_1.31.1:=70913edaf2263a157393af07565c17f0
$(PKG)_SOURCE_MD5_1.32.0:=9576986f1a960da471d03b72a62f13c7
$(PKG)_SOURCE_MD5:=$($(PKG)_SOURCE_MD5_$($(PKG)_VERSION))
$(PKG)_SITE:=http://www.busybox.net/downloads

$(PKG)_REBUILD_SUBOPTS += FREETZ_BUSYBOX__VERSION_V127
$(PKG)_REBUILD_SUBOPTS += FREETZ_BUSYBOX__VERSION_V131
$(PKG)_REBUILD_SUBOPTS += FREETZ_BUSYBOX__VERSION_V132
...
$($(PKG)_TARGET_BINARY).links: $($(PKG)_BINARY).links
	cat $(BUSYBOX_BINARY).links | sed -r -e 's,($(if $(FREETZ_AVM_HAS_REBOOT_SCRIPT),reboot|)$(if $(FREETZ_AVM_HAS_IP_BINARY),ip|)blkid|wget),\1-busybox,g' > $(BUSYBOX_TARGET_BINARY).links
