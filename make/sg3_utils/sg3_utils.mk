...
SG3UTILS_TARGET_DIR:=$(PACKAGES_DIR)/$(SG3UTILS_PKG_NAME)/root/usr/bin
SG3UTILS_TARGET_BINARY:=sg_start

$(DL_DIR)/$(SG3UTILS_SOURCE):
	wget -P $(DL_DIR) $(SG3UTILS_SITE)/$(SG3UTILS_SOURCE)

$(DL_DIR)/$(SG3UTILS_PKG_SOURCE):
...
$(SG3UTILS_DIR)/$(SG3UTILS_TARGET_BINARY): $(SG3UTILS_DIR)/.unpacked
	PATH="$(TARGET_PATH)" CFLAGS="$(TARGET_CFLAGS)" CC="$(TARGET_CC)" LD="$(TARGET_LD)" \
		make -f no_lib/Makefile.linux sg_start -C $(SG3UTILS_DIR)
	touch $@

$(PACKAGES_DIR)/.$(SG3UTILS_PKG_NAME): $(DL_DIR)/$(SG3UTILS_PKG_SOURCE)
...
sg3_utils-precompiled: uclibc $(SG3UTILS_DIR)/$(SG3UTILS_TARGET_BINARY) sg3_utils
	$(TARGET_STRIP) $(SG3UTILS_DIR)/$(SG3UTILS_TARGET_BINARY)
	cp $(SG3UTILS_DIR)/$(SG3UTILS_TARGET_BINARY) $(SG3UTILS_TARGET_DIR)
