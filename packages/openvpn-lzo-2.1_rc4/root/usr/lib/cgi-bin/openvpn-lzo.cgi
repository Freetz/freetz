...
keep_chk=''; comp_chk='';
tun_chk=''; tap_chk='';
static_chk=''; certs_chk='';
redir_chk=''; client2client_chk='';
bf_chk=''; aes128_chk=''; aes256_chk='';
des3_chk=''; float_chk=''; logfile_chk='';
dhcpclient_chk=''; pull_chk=''; tlsauth_chk='';

if [ "$OPENVPN_LZO_ENABLED" = "yes" ]; then auto_chk=' checked'; else man_chk=' checked'; fi
if [ "$OPENVPN_LZO_MODE" = "server" ]; then server_chk=' checked'; else client_chk=' checked'; fi
if [ "$OPENVPN_LZO_PROTO" = "udp" ]; then udp_chk=' checked'; else tcp_chk=' checked'; fi
if [ "$OPENVPN_LZO_KEEPALIVE" = "yes" ]; then keep_chk=' checked'; fi
if [ "$OPENVPN_LZO_COMPLZO" = "yes" ]; then comp_chk=' checked'; fi
if [ "$OPENVPN_LZO_TYPE" = "tap" ]; then tap_chk=' checked'; else tun_chk=' checked'; fi
if [ "$OPENVPN_LZO_AUTH_TYPE" = "certs" ]; then certs_chk=' checked'; else static_chk=' checked'; fi
if [ "$OPENVPN_LZO_PUSH_REDIRECT" = "yes" ]; then redir_chk=' checked'; fi
if [ "$OPENVPN_LZO_CLIENT2CLIENT" = "yes" ]; then client2client_chk=' checked'; fi
if [ "$OPENVPN_LZO_FLOAT" = "yes" ]; then float_chk=' checked'; fi
if [ "$OPENVPN_LZO_LOGFILE" = "yes" ]; then logfile_chk=' checked'; fi
if [ "$OPENVPN_LZO_DHCP_CLIENT" = "yes" ]; then dhcpclient_chk=' checked'; fi
if [ "$OPENVPN_LZO_PULL" = "yes" ]; then pull_chk=' checked'; fi
if [ "$OPENVPN_LZO_TLS_AUTH" = "yes" ]; then tlsauth_chk=' checked'; fi

case "$OPENVPN_LZO_CIPHER" in
...
<p>$(lang de:"Port" en:"Port"): <input id="port" type="text" size="4" maxlength="5" name="port" value="$(httpd -e "$OPENVPN_LZO_PORT")"> $(lang de:"Lokale Adresse" en:"Local Address") (optional): <input id="local" type="text" size="12" maxlength="16" name="local" value="$(httpd -e "$OPENVPN_LZO_LOCAL")"></p>
...
<p>$(lang de:"IP-Adresse" en:"IP-Address"): <input id="box_ip" size="12" maxlength="16" type="text" name="box_ip" value="$(httpd -e "$OPENVPN_LZO_BOX_IP")"> $(lang de:"Subnetzmaske" en:"Subnet Mask"): <input id="box_mask" size="12" maxlength="16" type="text" name="box_mask" value="$(httpd -e "$OPENVPN_LZO_BOX_MASK")"></p>
<div id="div_endpoint">
...
cat << EOF
<p>$(lang de:"Lokales Netzwerk" en:"Local Network Address"): <input id="local_net" size="30" maxlength="33" type="text" name="local_net" value="$(httpd -e "$OPENVPN_LZO_LOCAL_NET")"><br /><small>Syntax: &lt;ip&gt; &lt;subnetmask&gt;</small></p>
<p>$(lang de:"Entferntes Netzwerk" en:"Network Address"): <input id="remote_net" size="30" maxlength="33" type="text" name="remote_net" value="$(httpd -e "$OPENVPN_LZO_REMOTE_NET")"><br /><small>Syntax: &lt;ip&gt; &lt;subnetmask&gt;</small></p>
EOF
...
cat << EOF
<div id="div_dhcp">
<p>$(lang de:"Client Adressbereich (nur mit Zertifikaten)" en:"Client Address Range (only when used with Certs)"):<br /><input id="dhcp_range" type="text" size="30" maxlength="33" name="dhcp_range" value="$(httpd -e "$OPENVPN_LZO_DHCP_RANGE")"><br /><small>Syntax: &lt;start-ip&gt; &lt;end-ip&gt;</small></p>
</div>
<p>Max. Clients: <input id="maxclients" type="text" size="4" maxlength="3" name="maxclients" value="$(httpd -e "$OPENVPN_LZO_MAXCLIENTS")"></p>
<h2>Push $(lang de:"Optionen" en:"Options") (optional)</h2>
<p>DNS Server: <input id="push_dns" type="text" maxlength="16" name="push_dns" value="$(httpd -e "$OPENVPN_LZO_PUSH_DNS")"></p>
<p>WINS Server: <input id="push_wins" type="text" maxlength="16" name="push_wins" value="$(httpd -e "$OPENVPN_LZO_PUSH_WINS")"></p>
...
cat << EOF
<p>Server: <input id="remote" type="text" name="remote" value="$(httpd -e "$OPENVPN_LZO_REMOTE")"><br /><small>Server $(lang de:"Hostname oder IP-Adresse" en:"Hostname or IP-Address")</small></p>
...
<p>$(lang de:"Bandbreitenbegrenzung" en:"Traffic Shaping") (optional): <input id="shaper" type="text" name="shaper" size="4" maxlength="5" value="$(httpd -e "$OPENVPN_LZO_SHAPER")"><br /><small>B/s (Bytes $(lang de:"pro Sekunde" en:"per second"))</small></p>
