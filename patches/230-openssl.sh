if [ "$FREETZ_REPLACE_SSL_LIBS" == "y" ]; then
	if [ "$FREETZ_LIB_libcrypto" == "y" ]; then
		...
		ln -sf "/usr/lib/freetz/libcrypto.so.1.0.0" "${FILESYSTEM_MOD_DIR}/lib/libcrypto.so.0.9.8"
	fi
	if [ "$FREETZ_LIB_libssl" == "y" ]; then
		...
		ln -sf "/usr/lib/freetz/libssl.so.1.0.0" "${FILESYSTEM_MOD_DIR}/lib/libssl.so.0.9.8"
	fi
fi
