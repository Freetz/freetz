...
if [ "$FREETZ_CREATE_SEPARATE_OPTIONS_CFG" != "y" ]; then
	...
	OPTIONS_NAMES="$(grep -hoE "FREETZ_REPLACE_KERNEL|FREETZ_AVMDAEMON_DISABLE_[A-Z0-9]*|FREETZ_AVM_VERSION_[X0-9_]*(_MIN)?|FREETZ_(TARGET|BUSYBOX)_[A-Z0-9_]*|(EXTERNAL_)?FREETZ_(PACKAGE|LIB)[a-zA-Z0-9_]*" $OPTIONS_FILES | sort -u)"
