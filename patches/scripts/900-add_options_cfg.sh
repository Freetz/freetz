...
if [ "$FREETZ_CREATE_SEPARATE_OPTIONS_CFG" != "y" ]; then
	...
	OPTIONS_NAMES="$(grep -hoE "FREETZ_AVM_VERSION_[X0-9_]*|FREETZ_TARGET_[A-Z0-9_]*|FREETZ_REPLACE_KERNEL|FREETZ_AVMDAEMON_DISABLE_[A-Z0-9]*|EXTERNAL_FREETZ_PACKAGE[a-zA-Z0-9_]*|FREETZ_PACKAGE[a-zA-Z0-9_]*" $OPTIONS_FILES | sort -u)"
