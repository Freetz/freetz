...
if grep -q "cannot mount squashfs, trying ext2" "${var_install_file}" 2>/dev/null; then
	echo2 "adding alternative offset-based ext2-mount-method (reduces memory footprint while flashing the firmware)"
