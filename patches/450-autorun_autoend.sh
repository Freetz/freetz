...
if  [ "$FREETZ_TYPE_LABOR_DSL" ] || [ "$FREETZ_TYPE_WLAN_3270" ]; then
	modpatch "$FILESYSTEM_MOD_DIR" "${PATCHES_DIR}/cond/450-autorun-run_mount_3270.patch"
elif [ "$FREETZ_TYPE_FON_WLAN_7240" ] || [ "$FREETZ_TYPE_FON_WLAN_7270" ] || [ "$FREETZ_TYPE_LABOR_ALL" ]; then
...
if  [ "$FREETZ_TYPE_FON_WLAN_7240" ] || [ "$FREETZ_TYPE_FON_WLAN_7270" ] || [ "$FREETZ_TYPE_LABOR_ALL" ]; then
