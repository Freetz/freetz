...
$(KERNEL_CROSS_COMPILER): $(DL_DIR)/$(KERNEL_TOOLCHAIN_SOURCE) | $(KERNEL_TOOLCHAIN_SYMLINK_DOT_FILE) busybox-host tar-host
	mkdir -p $(TOOLCHAIN_DIR)/build
	$(RM) -r $(TOOLCHAIN_BUILD_DIR)/$(KERNEL_TOOLCHAIN_COMPILER)
	$(call UNPACK_TARBALL,$(DL_DIR)/$(KERNEL_TOOLCHAIN_SOURCE),$(TOOLCHAIN_DIR)/build)
	@touch $@

gcc: $(TARGET_CROSS_COMPILER)
$(TARGET_CROSS_COMPILER): $(DL_DIR)/$(TARGET_TOOLCHAIN_SOURCE) | $(TARGET_TOOLCHAIN_SYMLINK_DOT_FILE) busybox-host tar-host
	mkdir -p $(TOOLCHAIN_DIR)/build
	$(RM) -r $(TOOLCHAIN_BUILD_DIR)/$(TARGET_TOOLCHAIN_COMPILER)
	$(call UNPACK_TARBALL,$(DL_DIR)/$(TARGET_TOOLCHAIN_SOURCE),$(TOOLCHAIN_DIR)/build)
	@touch $@
