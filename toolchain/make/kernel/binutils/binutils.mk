...
ifeq ($(KERNEL_TOOLCHAIN_BINUTILS_MAJOR_VERSION),2.20)
...
ifeq ($(BINUTILS_KERNEL_VERSION),2.18)
BINUTILS_KERNEL_MD5:=9d22ee4dafa3a194457caf4706f9cf01
endif
ifeq ($(BINUTILS_KERNEL_VERSION),2.20.51.0.12)
BINUTILS_KERNEL_MD5:=f3a1f0535935ddae61ded2bcd3c7ce09
endif

# We do not rely on the host's gmp/mpfr but use a known working one
BINUTILS_KERNEL_HOST_PREREQ=

ifndef KERNEL_TOOLCHAIN_NO_MPFR
BINUTILS_KERNEL_HOST_PREREQ+=$(GMP_HOST_BINARY) $(MPFR_HOST_BINARY)

BINUTILS_KERNEL_EXTRA_CONFIG_OPTIONS+=--with-gmp=$(GMP_HOST_DIR)
BINUTILS_KERNEL_EXTRA_CONFIG_OPTIONS+=--with-mpfr=$(MPFR_HOST_DIR)
endif
...
$(BINUTILS_KERNEL_DIR1)/.configured: $(BINUTILS_KERNEL_DIR)/.unpacked $(BINUTILS_KERNEL_HOST_PREREQ)
	...
	(cd $(BINUTILS_KERNEL_DIR1); \
		...
		--disable-werror \
		--without-headers \
		$(BINUTILS_KERNEL_EXTRA_CONFIG_OPTIONS) \
		$(QUIET) \
	);
	touch $@
