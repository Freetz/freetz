...
if [ "$BFTPD_ZLIB_ENABLED" = "yes" ]; then auto_chk=' checked'; else man_chk=' checked'; fi
if [ "$BFTPD_ZLIB_ANONYMOUS" = "yes" ]; then anonym_chk=' checked'; fi
...
<p>Port: <input type="text" name="port" size="5" maxlength="5" value="$(httpd -e "$BFTPD_ZLIB_PORT")"></p>
<h2>$(lang de:"Ports f&uuml;r passives FTP, z.B.: 5000,6000-6100" en:"Port range for passive FTP, e.g. 5000,6000-6100"):</h2>
<p>$(lang de:"Ports" en:"Port range"): <input type="text" name="passive_ports" size="30" maxlength="255" value="$(httpd -e "$BFTPD_ZLIB_PASSIVE_PORTS")"></p>
<h2>$(lang de:"Zus&auml;tzliche Kommandozeilen-Optionen (f&uuml;r Experten)" en:"Additional command-line options (for experts)"):</h2>
<p>$(lang de:"Optionen" en:"Options"): <input type="text" name="options" size="20" maxlength="255" value="$(httpd -e "$BFTPD_ZLIB_OPTIONS")"></p>
EOF
